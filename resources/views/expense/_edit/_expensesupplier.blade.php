  ...
  <div class="col-xxl-12">
    ...
    <table class="table form-table" id="supplierFields">
      ...
      <tbody>
        <tr valign="top">
          <td width="30%">
            <select class="form-control supplier-dropdown" name="supplier_id" id="supplier">
              <option value="" disabled selected>--Select Supplier--</option>
              @foreach($suppliers as $sup)
              <option value="{{ $sup->id }}"
                {{ optional($stockSuppliers)->supplier_id == $sup->id ? 'selected' : '' }}>
                {{ $sup->brand }}
              </option>
              @endforeach
            </select>
          ...
          <td width="20%">
            <select class="form-control" name="price_basis">
              ...
              <option value="{{ $cur->code }}"
                {{ optional($stockSuppliers)->price_basis == $cur->code ? 'selected' : '' }}>
                {{ strtoupper($cur->code) }}
              </option>
              @endforeach
            </select>
          ...
          <td width="20%">
            <select class="form-control" name="delivery_term">
              ...
              <option value="{{ $term->short_code }}"
                {{ optional($stockSuppliers)->delivery_term == $term->short_code ? 'selected' : '' }}>
                {{ $term->title }}
              </option>
              @endforeach
            </select>
          ...
          <td width="30%">
            <textarea rows="2" name="supplier_remark" placeholder="Supplier Remarks" class="form-control">
              {{ optional($stockSuppliers)->remarks }}
            </textarea>
          </td>
          ...
          <tr valign="top">
            ...
            <td width="20%">
              <select class="form-control" name="status[]">
                <option value="" {{ $payment->status === null ? 'selected' : '' }}>--Select--</option>

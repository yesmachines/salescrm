...
<div class="row">
  <div class="container" style="padding-top: 25px;">
    <div class="col-sm-12">
      <button type="button" onclick="addRow()" style="background-color: #007D88; color: white; padding: 4px 9px; border: none; border-radius: 4px; cursor: pointer;margin-bottom: 11px">Add Row</button>
      <div class="row align-items-end">
        <div class="col-sm-3">
          <p style="font-size: 15px; color: black; font-weight: bold;">Model/Part Number</p>
        </div>
        <div class="col-sm-2">
          <p class="currency-label" style="font-size: 15px; color: black; font-weight: bold;">Unit Price(AED)</p>

        </div>
        <div class="col-sm-1">
          <p style="font-size: 15px; color: black; font-weight: bold;">Quantity</p>
        </div>
        <div class="col-sm-2">
          <p class="currency-label" style="font-size: 15px; color: black; font-weight: bold;">Line Total(AED)</p>
        </div>
        <div class="col-sm-1">
          <p style="font-size: 15px; color: black; font-weight: bold;">Discount(%)</p>
        </div>
        <div class="col-sm-2" style="white-space: nowrap;">
          <p class="currency-label" style="font-size: 15px; color: black; font-weight: bold;">Total After Discount(AED)</p>
        </div>
        <div class="col-sm-1">
        </div>
      </div>
      <hr class="my-1" style="border: 0; border-top: 2px solid #007D88; margin: 2px 0;">
        ...
        <div id="itemsContainer" style="margin-top: 12px;">
          ...
          <div class="row" style="margin-top: 12px;">
            <div class="col-sm-3">
              ...
              <div style="margin-top: 12px;">
                <textarea class="form-control product-description" name="item_description[]" placeholder="Description" rows="2"></textarea>
              </div>
              <div class="customCurrencySection" style="--bs-gutter-x: 0; display: none; padding-top: 3px;">
                <div class="d-flex">
                  ...
                  <button id="applyCurrency" class="form-control ms-2" onclick="applyConversionRate()" style="background-color: #007D88; color: white;">Apply</button>
                </div>
                <p class="text-warning"><small>If your price in AED. Please apply conversion to other currencies.</small></p>
              </div>
            </div>
            <div class="col-sm-2">
          ...
          <div class="row" style="margin-top: 12px;" data-item-id="{{ $quotationItem->id }}">
            <div class="col-sm-2">
              <select name="product[]" class="form-select category-select">
                <option value="products" {{ isset($quotationItem->product->product_category) && $quotationItem->product->product_category === 'products' ? 'selected' : '' }}>Products</option>

                <option value="accessories" {{ isset($quotationItem->product->product_category) && $quotationItem->product->product_category === 'accessories' ? 'selected' : '' }}>Accessories</option>
                <option value="consumables" {{ isset($quotationItem->product->product_category) && $quotationItem->product->product_category === 'consumables' ? 'selected' : '' }}>Consumables</option>
                <option value="spares" {{ isset($quotationItem->product->product_category) && $quotationItem->product->product_category === 'spares' ? 'selected' : '' }}>Spares</option>
              </select>
              <div style="margin-top: 12px;">
                <select name="item_id[]" class="form-select modelno-select" placeholder="Model"></select>
                <input type="hidden" name="item_ids[]" value="{{$quotationItem->item_id}}">
                <input type="hidden" name="category[]" value="{{ isset($quotationItem->product->product_category) ? $quotationItem->product->product_category : '' }}">
              </div>
              <div style="margin-top: 12px;">
                <textarea class="form-control product-description" name="item_description[]" placeholder="Description" rows="2">{{ $quotationItem->description }}</textarea>
              </div>
              <div class="customCurrencySection" style="--bs-gutter-x: 0; display: none; padding-top: 3px;">
                <div class="d-flex">
                  <input type="text" class="currencyConvertion form-control" placeholder="Conversion Rate" name="currency_convertion[]">
            ...
            <div class="col-sm-2">
              <input type="text" name="unit_price[]" placeholder="Unit Price" class="form-control unit-price" readonly value="{{ number_format($quotationItem->unit_price, 2, '.', '') }}">
              <div style="margin-top: 12px;">
                <span class="display-margin-price"></span>
                <span class="display-margin-percent"></span>
                <input type="hidden" name="margin_price[]" class="form-control margin-price" placeholder="Margin Price" readonly value="{{ isset($quotationItem->product->margin_price) ? $quotationItem->product->margin_price : '' }}">
                <input type="hidden" name="quote_margin_price[]" class="form-control quote-margin-price" value="{{ isset($quotationItem->margin_price) ? $quotationItem->margin_price : '' }}">
                <input type="hidden" name="margin_percent[]" placeholder="Margin" class="form-control margin-percent" value="{{ isset($quotationItem->product->margin_percent) ? $quotationItem->product->margin_percent : '' }}">
                <input type="hidden" name="product_currency[]"  class="form-control product-currency">
               <input type="hidden" name="product_selling[]"  class="form-control product-selling">
               <input type="hidden" name="product_margin[]"  class="form-control product-margin">
              <input type="hidden" name="product_percent[]"  class="form-control product-percent">
            ...
            <div class="col-sm-1">
              <input type="hidden" name="suppliers_value[]" class="form-control suppliersValue" value="{{ $quotationItem->supplier->brand ?? '' }}">
              <input type="number" name="quantity[]" placeholder="Quantity" class="form-control quantity" value="{{ $quotationItem->quantity ?? '' }}">

            </div>
            <div class="col-sm-2">
              <input type="number" name="subtotal[]" placeholder="Subtotal" class="form-control subtotal" readonly value="{{ isset($quotationItem->subtotal) ? number_format($quotationItem->subtotal, 2, '.', '') : '' }}">
            </div>
            <div class="col-sm-2">
              <input type="number" name="discount[]" placeholder="Discount" class="form-control discount" value="{{ $quotationItem->discount ?? '' }}" min="0">
            </div>
            <div class="col-sm-2">
              <input type="number" name="total_after_discount[]" placeholder="Total after Discount" class="form-control total" readonly value="{{ isset($quotationItem->total_after_discount) ? number_format($quotationItem->total_after_discount, 2, '.', '') : '' }}">
              <input type="hidden" name="margin_amount_row[]" placeholder="Margin" class="form-control margin-amount-row" value="{{ $quotation->gross_margin }}">
              <div style="margin-top: 12px;">
                <span class="quote-margin-price"></span>
              </div>
            </div>
            <div class="col-sm-1">
              <button type="button" onclick="removeQuotationRow(this)" style="color: #007D88; border: 1px solid #007D88; border-radius: 4px; background-color: transparent; padding: 4px 8px; cursor: pointer;">
...
<div class="row" style="margin-top: 21px;margin-left:89px;">
  ...
  <div class="col-sm-3" style="text-align: left;">
    <input type="hidden" name="initial_total_amount" value="{{ $quotation->total_amount }}">
...
<div class="row" style="margin-top: 21px">
  ...
  <div class="col-sm-1 text-align-left" style="width: 11%;">
    <label> <input type="radio" id="delivery-terms1" name="delivery_terms" value="is_stock" required <?= (isset($deliveryTerms->title) && $deliveryTerms->title === 'is_stock') ? 'checked' : '' ?>> In Stock</label>
  </div>
  <div class="col-sm-2" style="text-align: left;">
    <label> <input type="radio" id="delivery-terms2" name="delivery_terms" value="out_stock" <?= (isset($deliveryTerms->title) && $deliveryTerms->title === 'out_stock') ? 'checked' : '' ?>> Out of stock</label>
  </div>
...
<div>
  ...
  <div class="row" style="margin-top: 21px;">
    ...
    <div class="col-sm-2">
      <div>
        <input class="form-control" type="number" id="delivery-terms-info1" name="delivery_weeks" value="{{ isset($deliveryTerms->description) ? $deliveryTerms->description : '' }}" name="delivery_weeks">
      </div>
    </div>
  </div>
...
<div id="payment-cycles">
    ...
    <div class="col-sm-2">
      <div style="text-align: left;">
        <input type="text" class="form-control" value="{{ $paymentCycle->description }}" name="payment_description[]" class="form-control">
      </div>
    </div>
...
<div class="row" data-term-id="{{ $quotationTerm->id}}">
  ...
  @else
  <div>
    @foreach($quotationTermsList as $index=> $quotationTermsData)
    <div class="row">
      <div class="col-md-5">
        <div class="form-group">
  ...
  <div class="modal-dialog modal-dialog-centered modal-xl">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div id="additionalText" style="cursor: pointer; color: #007D88;" data-bs-toggle="modal" data-bs-target="#additionalFieldsModal">Do you want to change the currency?</div>
        <div class="modal-footer">
...
<div class="modal fade" id="additionalFieldsModal" tabindex="-1" aria-labelledby="additionalFieldsModalLabel" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="additionalFieldsModalLabel">Update Currency</h5>
...
$(document).ready(function() {
  $('#delivery-terms-info1, #delivery-terms1, #delivery-terms2').on('input', function() {
    var weeksValue = $('#delivery-terms-info1').val();
    var selectedValue = $('#delivery-terms1').is(':checked') ? 'In Stock' : 'Out of Stock';

    var deliveryText = selectedValue + '. ' + weeksValue + ' working weeks from the date of po & advance payment if any';
...
function addRow() {
  ...
  <div style="margin-top: 12px;">
  <select name="item_id[]" class="form-select modelno-select" placeholder="Model" id="modelDropdown"></select>
  </div>
  <div style="margin-top: 12px;">
  <textarea class="form-control product-description" name="item_description[]" placeholder="Description" rows="2"></textarea>
  </div>
...
(document).ready(function() {
  ...
  $(document).on('click', '.select2-selection__choice__remove', function(event) {
    ...
    var parentTitle = $(this).parent().attr('title').trim();
    if (parentTitle) {
      var optionValue;
      var colonIndex = parentTitle.indexOf(":");

      if (colonIndex !== -1) {
        optionValue = parentTitle.substring(colonIndex + 1).trim();
      } else {
        optionValue = parentTitle;
      }

      var rows = $('#itemsContainer').find('.row');

      // Array to store rows to remove
      var rowsToRemove = [];

      // Loop through rows to find the rows belonging to the same supplier
      rows.each(function() {
        var row = $(this);

        row.find('.suppliersValue').each(function() {
          var suppliersValue = $(this).val();
          if (suppliersValue === optionValue) {
            rowsToRemove.push(row);
          }
        });
      });

      // Remove the identified rows if found
      if (rowsToRemove.length > 0) {
        rowsToRemove.forEach(function(rowToRemove) {
          rowToRemove.remove();
        });

        // Update selected model IDs and fetch models
        selectedModelIds = collectSelectedModelIds();
        var selectedSupplierId = $('select[name="supplier_id[]"]').val();
        fetchModels(selectedCategories, selectedSupplierId, selectedModelIds);
      } else {

      }
    } else {
      console.log("Parent Title is undefined");
    }
  });

  // Function to collect selected model IDs
  function collectSelectedModelIds() {
    var selectedModelIds = [];
    $('.modelno-select').each(function() {
      var selectedId = $(this).val();
      selectedModelIds.push(selectedId);
    });

...
class ClosedOrders extends Component
{
  ...
  public function render(Request $request, OrderService $orderService)
  {
    $query = $this->getClosedQuery($request, $orderService);
    $orders = $query->paginate($this->perPage, ['*'], 'page', $this->page);  // Manually set the current page
    return view('livewire.closed-orders', ['orders' => $orders]);
  }
  public function getClosedQuery(Request $request, OrderService $orderService)
  {
    $search = $this->search;

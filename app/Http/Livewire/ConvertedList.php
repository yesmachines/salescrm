...
class ConvertedList extends Component
{
  public $page = 1;
  use WithPagination;
  ...
  public function render(Request $request, LeadService $leadService, EmployeeManagerService $employeeManagerService)
  {
    $query = $this->getConvertedQuery($request, $leadService, $employeeManagerService);
    $leads = $query->paginate($this->perPage, ['*'], 'page', $this->page);  // Manually set the current page
    return view('livewire.converted-list', ['leads' => $leads]);
  }
  public function getConvertedQuery(Request $request, LeadService $leadService, EmployeeManagerService $employeeManagerService)
  {
    $input = $request->all();
    $search = $this->search;
    $arrIds = [];
    ...
    if (isset($input['enquiry_start_date']) && isset($input['enquiry_end_date'])) {
      $query->whereBetween('enquiry_date', array($input['enquiry_start_date'], $input['enquiry_end_date']));
    }

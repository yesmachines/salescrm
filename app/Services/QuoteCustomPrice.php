...
if (isset($userData['charge_name']) && isset($userData['charge_amount'])) {
    ...
    for ($i = 0; $i < count($chargeNames); $i++) {
        ...
        if (!is_null($chargeAmount) && $chargeAmount !== '') {
            ...
            $chargeData = [
                'quotation_id' => $quotationId,  // Quotation ID
                'title' => $chargeName,  // Charge name
                'amount' => $chargeAmount,  // Charge amount
                'short_code' => $shortcodeTitle,  // Charge short code
                'quote_visible' => $isVisible,  // Visibility flag (1 for visible)
            ];

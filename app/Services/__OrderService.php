...
class OrderService
{
    ...
    public function insertOrder(array $userData, $insert = []): void
    {
        if (!empty($insert)) {
            foreach ($insert as $input) {
                $ifExist =  Orders::where('yespo_no', '=', $input['yespo_no'])->count();
                if (!$ifExist) {
                    Orders::create([
                        'company_id'        => $userData['company_id'],
                        'customer_id'       => $userData['customer_id'],
                        'yespo_no'          => $input['yespo_no'],
                        'po_number'         => (isset($input['pono']) && !is_null($input['pono'])) ? $input['pono'] : '',
                        'po_date'           => (isset($input['po_date']) && !is_null($input['po_date'])) ? $input['po_date'] : '',
                        'po_received'       => (isset($input['po_received']) && !is_null($input['po_received'])) ? $input['po_received'] : '',
                        'short_link_code'   => strtotime("now"),
                        'status'            => 'open',
                    ]);
                } else {
                    continue;

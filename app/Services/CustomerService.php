...
class CustomerService
{
  ...
  {
    $keywords = trim($userData['fullname']);
    $email    = trim($userData['email']);

    $sql = Customer::where('company_id', $userData['company_id'])
      ->where(function ($query) use ($keywords, $email) {
  ...
  {
    $keywords = trim($userData['company']);
    ...
    if ($sql->count() > 0) {
      ...
      $company = Company::create([
        ...
        'region_id'     => isset($userData['region_id']) ? $userData['region_id'] : 0,
